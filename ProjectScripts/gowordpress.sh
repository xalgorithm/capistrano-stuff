...
then
    echo "usage: gowordpress.sh -n {app_name} -r {repo_name} -s {safe_name} -d {display_name} -p {db_pass}
    e.g. gowordpress.sh -n pathlabsofark -r PathLabsOfArk -s pathlabsofark -d \"Pathology Labs of Arkansas\" -p \"KdjEhvieELp\"" 1>&2
    exit 1
fi
...
then
    ...
    ./createRepoFromTemplate.sh WordpressTemplate $REPO_NAME
fi
